...
def pregunta_01():
    ...
    y_reshaped = y.reshape(-1, 1) #np.array(y)

    # Trasforme `X` a un array de numpy usando reshape
    X_reshaped = X.reshape(-1, 1) #np.array(X)
...
def pregunta_03():
    ...
    X_fertility = np.array(df["fertility"].values).reshape(-1,1)

    # Asigne a la variable los valores de la columna `life`
    y_life = np.array(df["life"].values).reshape(-1,1)
    #print(np.array([[1, 1], [1, 2], [2, 2], [2, 3]]))
...
def pregunta_04():
    ...
    (X_train, X_test, y_train, y_test,) = train_test_split(X_fertility,y_life ,
                                   random_state=53, 
                                   test_size=0.2)
    ...
    print("R^2: {:6.4f}".format(lg.score(X_test, y_test)))
